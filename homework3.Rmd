...
member_payments %>% 
  ...
  group_by(name) %>% 
  summarize(total_donations_Withers=sum(value)) %>% 
  arrange(desc(total_donations_Withers)
```
...
cdc_data %>% 
  ...
  filter(death_yn =="Yes") %>% 
  mutate(icu_reformatted = case_when(icu_yn=="Yes" ~ "ICU Admission", TRUE ~"No ICU Admission")) %>% 
  ggplot(aes(x = percentage, y =age_group, fill=death_yn)) +
  geom_bar(stat="identity") +
  facet_grid(factor(icu_reformatted, levels = c("ICU Admission","No ICU Admission"))~sex) + theme_light() +
  labs(x="",y="", caption = "Source: CDC",
       title="Covid CFR % by age group, sex and ICU Admission") +
  scale_x_continuous(labels=scales::percent) +
  guides(fill="none") +
  geom_text((aes(label=(sprintf("%0.0f",percentage*100)))), size=3, hjust=1, color= "black") + scale_y_discrete(limits = c("0 - 17 years", "18 to 49 years", "50 to 64 years", "65+ years"))
```
...
***ASK KOSTIS ABOUT WORD VS HTML AND DISCREPANCY IN Q1 PAYMENTS VS DONATIONS AND GRAPH SIMPLIFY***
